...
namespace Wolfje.Plugins.SEconomy.Journal {
    ...
    /// </summary>
    public sealed partial class XmlTransactionJournal : ITransactionJournal {
        volatile bool JournalBackingUp;
        public const string kXMLTransactionJournalSavePath = "XMLJournalSavePath";
        ...
        /// <summary>
        /// This is a dictionary of constant keys specified for configuring this instance of an ITransactionJournal
        /// </summary>
        ...
        /// </summary>
        public readonly Version XmlSchemaVersion = new Version(1, 2, 0);

        public bool JournalSaving { get; set; }

        public XmlTransactionJournal(Dictionary<string, object> Configuration)
        {
...
";
        ...
        public void DumpSummary()
        {
            ...
            //for (int i = 0; i < bankAccounts.Max(x => x.BankAccountK); i++) {
            //    sb.AppendLine(string.Format("{0},{1}", i, GetBankAccountList(i).Count()));
            //}
		...
		{
			try {
				string path = configuration[kXMLTransactionJournalSavePath] as string;
        ...
        {
            return Task.Factory.StartNew(() => BackupJournal());
        }

        public void SquashJournal()
        {
            ...
            SaveJournalAsync().ContinueWith((task) => {
        ...
        {
            return Task.Factory.StartNew(() => SquashJournal());
        }
        ...
        public BankTransferEventArgs TransferBetween(IBankAccount FromAccount, IBankAccount ToAccount, Money Amount, BankAccountTransferOptions Options, string TransactionMessage, string JournalMessage)
        {
            ...
            try {
                if (ToAccount != null) {
                    if (TransferMaySucceed(FromAccount, ToAccount, Amount, Options)) {
                        ...
                        if (SEconomyPlugin.Configuration.EnableProfiler) {
                            profile = SEconomyPlugin.Profiler.Enter(string.Format("transfer: {0} to {1}", !string.IsNullOrEmpty(FromAccount.UserAccountName) ? FromAccount.UserAccountName : "Unknown", ToAccount != null && !string.IsNullOrEmpty(ToAccount.UserAccountName) ? ToAccount.UserAccountName : "Unknown"));
                        }
                        ...
                        if (!ToAccount.IsSystemAccount && !ToAccount.IsPluginAccount) {
                            if (FromAccount.Owner != null) {
                                if (Amount < 0) {
                                    FromAccount.Owner.TSPlayer.SendErrorMessageFormat("Invalid amount.");
                                } else {
                                    FromAccount.Owner.TSPlayer.SendErrorMessageFormat("You need {0} more money to make this payment.", ((Money)(FromAccount.Balance - Amount)).ToLongString());
                                }
                            }
                        }
                    }
